...
One of reasons that research and development of GPGPU has been attracted pay attention is that it becomes popular that machine learning using libraries is applied to social implementation, and it requires more performance. The de facto standard programming language of machine learning is Python \cite{Python}. However, current GPGPU libraries such as CUDA \cite{CUDA}, CuPy \cite{CuPy}, TensorFlow \cite{TensorFlow} and Keras \cite{Keras}, which are based on Python, have following two problems:
